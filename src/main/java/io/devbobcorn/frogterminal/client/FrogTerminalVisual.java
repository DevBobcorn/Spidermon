...
public class FrogTerminalVisual extends AbstractBlockEntityVisual<FrogTerminalBlockEntity> implements SimpleDynamicVisual {
	...
	public FrogTerminalVisual(VisualizationContext ctx, FrogTerminalBlockEntity blockEntity, float partialTick) {
		super(ctx, blockEntity, partialTick);

		body = ctx.instancerProvider()
			.instancer(InstanceTypes.TRANSFORMED, Models.partial(FrogTerminalPartialModels.FROG_TERMINAL_BODY))
			.createInstance();

		head = ctx.instancerProvider()
			.instancer(InstanceTypes.TRANSFORMED, Models.partial(FrogTerminalPartialModels.FROG_TERMINAL_HEAD))
			.createInstance();

		tongue = ctx.instancerProvider()
			.instancer(InstanceTypes.TRANSFORMED, Models.partial(FrogTerminalPartialModels.FROG_TERMINAL_TONGUE))
			.createInstance();
	...
	public void updateGoggles() {
		if (blockEntity.goggles && !lastGoggles) {
			...
			head = instancerProvider()
				.instancer(InstanceTypes.TRANSFORMED, Models.partial(FrogTerminalPartialModels.FROG_TERMINAL_HEAD_GOGGLES))
				.createInstance();
			lastHeadPitch = -1;
			updateLight(0);
			lastGoggles = true;
